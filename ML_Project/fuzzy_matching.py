...
class FuzzyMatching:
    ...
    def clean_column(
            ...
            Uses class attributes if valid_values or alias_map are not provided.
            """
            # Use stored valid values if not explicitly provided
            if column_name.lower() == "brand":
                valid_values = valid_values or self.brand_list
                alias_map = alias_map or self.brand_aliases

            elif column_name.lower() == "transmission":
                valid_values = valid_values or self.transmission_list
                alias_map = alias_map or self.transmission_aliases
    ...
    def clean_modelCol(self, model_col="Model", brand_col="Brand", cutoff: int = 86):
        """
